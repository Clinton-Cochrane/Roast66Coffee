...
function DuplicateOrderPage() {
  ...
  const state = location.state as
    | { order?: OrderDto; existingOrderId?: number }
    | null
    | undefined;
  ...
  return (
    <div className="p-6 max-w-lg mx-auto text-center">
      ...
      <h1 className="text-2xl font-bold text-gray-800 mb-2">{t("duplicateOrder.title")}</h1>
      ...
      {existingOrderId ? (
        <div className="bg-amber-50 border border-amber-200 rounded-lg p-4 mb-6 text-left">
          ...
          <p className="text-sm text-amber-800 mt-1">{t("duplicateOrder.confirmedOrderHelp")}</p>
        </div>
      ) : null}
